"""Modulo contenente la classe dell'estrattore di entità tramite servizi azure"""
...
class OpenaiExtractor(EntityExtractor):
    """
    Classe associata alla definizione dell'estrattore di entità tramite il servizio Azure OpenAI.
    ...
    Attributi:
    provider (str): Il fornitore del servizio di estrazione. Default a "azure".
    service (str): Il servizio di estrazione utilizzato. Default a "openai".
    embedder_id_def (str): L'identificativo del modello di embedding utilizzato.
    embedder_name_def (str): Il nome della risorsa per il modello di embedding.
    embedder_api_version_def (str): La versione dell'API del modello di embedding.
    ...
    model_def (str): L'identificativo del modello linguistico utilizzato.
    api_version_def (str): La versione dell'API utilizzata per il modello.
    """
    ...
    def _extract_pdf_entities(
        ...
        list_img = []
        list_pages = []
        # start splitting, end when all chunk are computed
