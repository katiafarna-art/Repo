"""Modulo contenente la classe concreta per lo strumento ocr DocumnetIntelligence (Azure)"""

import requests
from typing import Optional, Union, Literal
from config.parameters import ETA_PAGE_LAI, DocumentIntelligenceDefaultConfig
...
class DocumentIntelligenceOCR(AbstractOCR, AbstractCloudService):
    """
    Classe relativa all'estrazione di testo utilizzando i servizi OCR di Azure document intelligence.

    Questa classe consente l'estrazione di testo a partire da file immagine o pdf utilizzando uno degli
    ...
    * model_name (str): Nome del modello OCR utilizzato. Il default (alla versione attuale del serviizio) è
      "prebuilt-layout".

    """

    model_name: str = DocumentIntelligenceDefaultConfig.model_id
    layerai_api: str = "docint-analyze?sv=1"
    layerai_file_key: str = "file"
    ...
    def extract_text_from_img(
        ...
        """
        Metodo per l'estrazione del testo da file immagine utilizzando i servizi OCR di Document Intelligence tramite
        layerai.

        :param token: jwt token contenente le informazioni necessarie per svolgere la chiamata al modello OCR tramite
            layerai.
        :type token: str
    ...
    def extract_text_from_pdf(
        ...
        """
        Metodo per l'estrazione del testo da pdf immagine utilizzando i servizi OCR di Document Intelligence tramite
        layerai.

        :param token: jwt token contenente le informazioni necessarie per svolgere la chiamata al modello OCR tramite
            layerai.
        :type token: str
        :param pdf: pdf su cui svolgere l'estrazione.
        :type pdf: bytes
        :param page_from: pagina del file da cui partire con l'estrazione.
        :type page_from: int
        :param page_to: pagina del file dove interrrompere l'estrazione.
        :type page_to: int
        :param use_case: use case di riferimento assegnato tramite layerai
        :type use_case: str
        :param params:  Dizionario contenente il nome del modello da impiegare per la chiamata ai servizi Azure Document
        ...
        :type: Literal["async", "sync"]
        """
        _, pages = self._configure_arguments(
            languages=None, page_from=page_from, page_to=page_to
        )
        model_name = self._extract_params(params=params)
        layer_ai_manager = LAIUtilitiesManager(token=token, use_case=use_case)
        ...
        files = {"file": ("uploaded_file.pdf", pdf, "application/pdf")}
        eta = ETA_PAGE_LAI * (
