"""Modulo contenente la classe concreta LM di Gemini (Google)"""
...
class GeminiLMLayerAI(AbstractLM):
    """
    Classe relativa all'estrazione di entità utilizzando i servizi LM Google.

    Descrizione: Questa classe consente di svolgere l'estrazione di entità a partire da contenuto testuale e da file
    immagine utilizzando i servizi LM forniti da Google.
    """


    def __init__(self, params: EntityExtractionParams) -> None:
        """
        Metodo di inizilizzazione della classe GeminiLM. Tale metodo richiama l'init della classe AbstractLM
        specificando il nome del provider e del servizio Gemini.
        """
        super().__init__(provider="google", service="gemini", params=params)
    ...
    def _process_content(content):
        ...
        for item in content:
            if item["type"] == "text":
                ...
                base64_data = item["image_url"].split(",", 1)[1]
                parts.append(
                    {"inline_data": {"data": base64_data, "mime_type": "image/png"}}
                )
            else:
                pass
    ...
    def _cast_to_google_payload(self, payload):

        messages = payload.get("messages", {}).get("messages", [])
        google_messages = []

        for message in messages:
            google_message = {
                "role": "USER",
                "parts": self._process_content(message["content"]),
            }
    ...
    def extract_entity_from_image(
        ...
        """
        Metodo per l'estrazione di entità da file immagine utilizzando i servizi LM di Google. Nello specifico,
        ...
        response_format = (
            "application/json" if response_format == "json_object" else response_format
        )

        for payload in payloads:

            google_payload = self._cast_to_google_payload(payload=payload)
            ...
            if isinstance(response_format, dict):
                is_structured = True
                other_payload["generation_params"].pop("response_format", None)
                other_payload["generation_params"]["response_schema"] = response_format
                other_payload["generation_params"]["response_mime_type"] = "application/json"
